...
object WithAsyncScript {
  class Props(val scripts: Set[String], _toRender: ⇒ VdomElement, val loadingScreenOpt: js.UndefOr[Set[String] => VdomElement]){
  ...
  val component =
    ScalaComponent.builder[Props]("AsyncLoadC")
    .initialState(Set.empty[String])
    .renderBackend[Backend]
    .componentWillMount($ => $.backend.load($.props, $.state))
    .build

  def apply(scripts: String*)(e: ⇒ VdomElement, loadingScreenOpt: js.UndefOr[Set[String] => VdomElement] = js.undefined) =
