...
object Octicons {
  ...
  val values = List(alert, arrow_down, arrow_left, arrow_right, arrow_small_down, arrow_small_left,
    arrow_small_right, arrow_small_up, arrow_up, beaker, bell,
    //    bold,
    bookmark, book, briefcase, broadcast,
    browser, bug, calendar, checklist, check, chevron_down, chevron_left, chevron_right, chevron_up,
    circle_slash, circuit_board, clippy, clock, cloud_download, cloud_upload, code, color_mode, comment_discussion,
    comment, credit_card, dashboard, dash, database, desktop_download, device_camera, device_camera_video, device_desktop,
    device_mobile, diff_added, diff_ignored, diff_modified, diff_removed, diff_renamed, diff, ellipsis, eye, file_binary,
    file_code, file_directory, file_media, file_pdf, file_submodule, file_symlink_directory, file_symlink_file, file_text,
    file_zip, flame, fold, gear, gift, gist_secret, gist, git_branch, git_commit, git_compare, git_merge, git_pull_request,
    globe, graph, heart, history, home, horizontal_rule, hubot,
    //    icons,
    inbox, info, issue_closed, issue_opened, issue_reopened,
    //    italic,
    jersey, keyboard, key, law, light_bulb, link_external, link, list_ordered, list_unordered, location, lock,
    logo_github, mail_read, mail_reply, mail, markdown, mark_github, megaphone, mention, milestone, mirror, mortar_board,
    primitive_square, pulse, question, quote, radio_tower, repo_clone, repo_force_push, repo_forked, repo_pull,
    repo_push, repo, rocket, rss, ruby, search, server, settings, shield, sign_in, sign_out, squirrel, star,
    stop, sync, tag,
    //    tasklist,
    telescope, terminal,
    //text_size,
