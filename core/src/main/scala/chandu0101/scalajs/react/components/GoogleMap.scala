...
object GoogleMap {
  ...
  class Backend(t: BackendScope[Props, State]) {
    ...
    def initialize(P: Props): Callback =
      t.root.getDOMNode.map(node =>
        t.modState(
          _.copy(mapObjects = Some((new GMap(node, MapOptions(P.center, P.zoom).toGMapOptions), new GInfoWindow))),
  ...
  val component = ScalaComponent.builder[Props]("googleMap")
    .initialState(State(None, Nil))
    .renderBackend[Backend]
    .componentWillReceiveProps {
      case componentWillReceiveProps => componentWillReceiveProps.backend.updateMap(componentWillReceiveProps.nextProps)
    }
