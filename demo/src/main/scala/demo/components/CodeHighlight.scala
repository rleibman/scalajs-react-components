...
object CodeHighlight {
  ...
  private val component = ScalaComponent.builder[String]("CodeHighLighter")
    .render_P(str => <.code(^.`class` := "scala", ^.padding := "20px", str))
    .configure(_.componentDidMountConst(applySyntaxHighlight).componentDidUpdateConst(applySyntaxHighlight))
    .build
