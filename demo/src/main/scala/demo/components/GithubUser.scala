...
object GithubUser {
  ...
  object Styles {
    val userGroup = TagMod(^.display := "inline-block", ^.textAlign := "center", ^.textDecoration := "none", ^.color := "black")

    val userIcon = TagMod(^.margin := "10px", ^.display := "block", ^.width := "100px", ^.height := "100px", ^.borderRadius := "50%")
  ...
  val component = ScalaComponent.builder[Props]("GithubUser")
